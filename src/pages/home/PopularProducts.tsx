...
const PopularCart = (props: { product: Product }) => {
  ...
  return (
    <div className=" w-full rounded-xl  bg-transparent relative group">
      <div
        className={`bg-gray-200 h-[460px] w-[450px] absolute -top-8 -left-6 rounded-full opacity-0 group-hover:opacity-100 duration-200 group-hover:scale-110`}
      ></div>
      <div className="relative">
        ...
        <div className={`text-center px-2 text-black w-full space-y-4`}>
          <h2 className="text-2xl font-austin ">{product?.name}</h2>
          <ProductPrice product={product} />
          <p className="w-[80%] m-auto text-md font-normal">
            Add a touch of royalty to any dish, with this versatile powder, made
            from nothing but
          </p>
          <div className="flex justify-center items-center w-full px-4">
            <button
              className={`px-5 py-3 bg-black text-white border-black rounded-full border-2 font-oswald hover:scale-110 duration-300 text-sm flex justify-center items-center gap-2`}
            >
  ...
  return (
    <div className="flex items-center justify-center font-roboto">
      <p className="flex justify-center items-center text-xl font-oswald">
        <>
          <TbCurrencyTaka />
          {product?.SKUs.map((item:any) => (item.active ? item.price : null))}
        </>
      </p>
...
const KgsButton = (props: { product: Product }) => {
  ...
  return (
    <div className="text-sm font-normal font-roboto flex justify-between items-center w-full">
      {product?.SKUs.map((item:any, index:number) => (
        <button
        key={index + ""}
